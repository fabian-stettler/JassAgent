...
class GameStateEvaluator:
    ...
    def create_state_key(self, state: GameState) -> tuple:
        """Generate hashable state key for memoization"""
        return (tuple(state.hands.flatten()), tuple(state.current_trick), 
                tuple(state.points), state.nr_cards_in_trick, 
                state.nr_played_cards, state.player, state.trump)